...
[ -f docker-compose.yml ] || die "run this from inside the amazon-inventory-sync folder"
...
echo
echo "Four questions. The answers come from your supplier's credentials sheet and your Amazon developer application."
...
# A shape, not a value -- see the matching comment in setup-env.ps1. A real
# username here would be worse than useless: an operator pasting the example
# instead of their own value connects to somebody else's feed and gets a green

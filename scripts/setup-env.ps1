<#
.SYNOPSIS
    Write the .env file, once, on a new Windows host.
...
if (-not (Test-Path -LiteralPath 'app\config.py')) {
    Stop-With "run this from inside the amazon-inventory-sync folder"
}
...
Write-Host ""
Write-Host "Four questions. The answers come from your supplier's credentials sheet and your Amazon developer application."
...
# The example is deliberately a shape, not a value. A real username here would
# be worse than useless: an operator who pastes the example instead of the
# value from their own credentials sheet would connect to somebody else's feed
# and get a green connection test for it.
...
$content = @"
# =============================================================================
# Amazon Inventory Sync - server configuration
# =============================================================================
...
# localhost. Create the role and database first; see docs/DEPLOYMENT.md.
...
# a starting guess. Credentials sheets commonly say "/" while the feeds sit
# one level down in a subfolder, and the symptom is a successful login that
# finds no files. Do not guess: press "Test the vendor connection"
...
Write-Host ""
Write-Host "Next: create the database role, then run the migrations. See docs/DEPLOYMENT.md." -ForegroundColor White
Write-Host ""

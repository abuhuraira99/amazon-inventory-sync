...
WHY IT MATTERS
==============
It runs with **no Amazon credentials and no network access**, against two
files anyone can export in a few minutes: a supplier feed and an Amazon All
Listings Report.

That is the point. It answers "how much of this catalogue could actually be
synced, and what would be missed?" *before* anybody agrees to let software
write to a seller account -- and the answer is worth having whether or not the
automation is ever switched on.
...
=====
    python scripts/stage0_coverage.py \
        ...
        --feed      "FULL_FEED_100000_20260903.zip" \
        --prefix    "EXAMPLE-"
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        action="append",
        default=None,
        help="SKU prefix in scope. Repeat for several. Defaults to EXAMPLE-.",
    )
    ap.add_argument("--max-quantity", type=int, default=999)
    ap.add_argument("--safety-buffer", type=int, default=0)
    ap.add_argument("--out-of-stock-at", type=int, default=0)
    ap.add_argument("--change-limit", type=int, default=2000)
    ap.add_argument("--json", type=Path, default=None, help="Write the report as JSON")
    args = ap.parse_args()

    prefixes: list[str] = args.prefix or ["EXAMPLE-"]
    ...
    print("   Barcodes are padded to 13 digits before the SKU is built.")
    print("   Below: what would happen WITHOUT padding.")
    print()
    ...
    # products that might be worth listing. The remainder -- the large
    # majority of any distributor feed -- are products the seller has never
    # listed AND the supplier has none of. Counting those as "unmatched" would
    # drown the real signal, which is "the supplier has stock of something we
    # could be selling".
    unmapped_in_stock = 0

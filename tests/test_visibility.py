...
====================
A background job can fail repeatedly and produce nothing at all -- no
dashboard alert, no error, no log -- and it takes only two ordinary-looking
decisions to make that possible.

  1. **Logging only to stdout.** Under Docker that IS the log and the platform
     captures it, which makes the choice look fine. Under a service manager
     that does not capture stdout -- a Windows Scheduled Task, a bare systemd
     unit, a cron entry -- every diagnostic the application produces is
  ...
  2. The refresh job caught one exception type and alerted on it. Everything
     ...
     stream and carried on. Catching one exception type and alerting on it is
     not error handling; it is error handling for the error you thought of.
...
class TestTheCatalogueRefreshAlwaysReportsBack:
    ...
    def test_missing_amazon_credentials_raise_an_alert(
        ...
        A warning is invisible wherever stdout is discarded, so an operator presses the
    ...
    def test_an_unexpected_failure_raises_an_alert_naming_its_type(
        ...
        assert _Client.closed, "the API client must still be closed on the failure path"
...
class TestARunNeverStaysRunningForever:
    ...
    a service manager stopping the job. The row keeps the status it was created
...
class TestAFailedRunCanRecordThatItFailed:
    ...
    A single NUL byte in a vendor feed aborts the

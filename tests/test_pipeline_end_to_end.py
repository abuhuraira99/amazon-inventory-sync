...
The feed rows and SKUs reproduce the real zero-stripping property, so padding is
genuinely exercised: the vendor sends ``25543055416`` and the account holds
``EXAMPLE-0025543055416``.
"""
...
from app.engine.mapping import CatalogIndex, ListingEntry
...
HEADER = "barcode|brand|title|price|stock|format"

#: Note the stripped leading zeros -- that is the vendor's own output. The
#: matching SKUs on the account are padded to 13 digits.
ROWS = [
    "25543055416|ACME|GIZMO (LARGE)|17.73|0|BULK",     # 131 -> 0
    "5319056434|CONTOSO|GADGET ASSORTED|9.84|6|STD",            # 2 -> 6
    "1115962442528|NORTHWIND|WIDGET STANDARD|12.12|4|STD",             # 4 -> 4, no change
]

#: barcode as the vendor sends it -> the SKU actually on the account
SKU_FOR = {
    "25543055416": "EXAMPLE-0025543055416",
    "5319056434": "EXAMPLE-0005319056434",
    "1115962442528": "EXAMPLE-1115962442528",
}
...
def feed_archive(tmp_path, session) -> Path:
    ...
    path = tmp_path / f"FULL_FEED_100000_{today}.zip"
...
def account(session) -> None:
    ...
    """
    for barcode, quantity in (("25543055416", 131), ("5319056434", 2), ("1115962442528", 4)):
        sku = SKU_FOR[barcode]
        session.add(
            AmazonListing(
                seller_sku=sku,
...
def toy_catalogue_guardrails(session):
    ...
    They are calibrated for a realistic catalogue, of which
...
def wired(session, feed_archive, monkeypatch, tmp_path):
    ...
    """
    # The shipped defaults are deliberately inert: no SKU prefix is in scope,
    # so a fresh install touches nothing at all until an operator says what it
    # owns. These tests are about the pipeline rather than about that default,
    # so they state the scope and quantity rules they exercise instead of
    # inheriting them. The default itself is asserted in TestScopeDefault.
    settings_store.set_value(session, "sku_prefixes_in_scope", ["EXAMPLE-"], actor="test")
    settings_store.set_value(session, "max_quantity", 15, actor="test")
    settings_store.set_value(session, "missing_full_feeds_before_zero", 1, actor="test")
    session.flush()
...
class TestPracticeMode:
    def test_a_full_run_decides_correctly_and_sends_nothing(
        ...
        """
        The rehearsal an operator is meant to spend weeks in. Every stage runs;
        no request reaches Amazon.

        Expected decisions from the fixture data:
            0025543055416  Amazon 131, vendor 0   -> change to 0
            0005319056434  Amazon   2, vendor 6   -> change to 6
            1115962442528  Amazon   4, vendor 4   -> no change
        """
        ...
        assert planned == {
            "EXAMPLE-0025543055416": (131, 0),
            "EXAMPLE-0005319056434": (2, 6),
        }
    ...
    def test_the_padding_rule_is_what_makes_the_match_work(
        ...
        """
        The whole project hinges on this. The vendor sends ``25543055416``; the
        SKU on the account is ``EXAMPLE-0025543055416``. Without padding to 13
        ...
        assert "EXAMPLE-0025543055416" in matched, (
            "the padded SKU was not matched: the naive form EXAMPLE-25543055416 "
    ...
    def test_the_vendor_data_is_stored_whatever_happens_to_amazon(
        ...
        and they produce the reports an operator relies on. They must
        ...
        assert stored["0025543055416"] == 0
        assert stored["0005319056434"] == 6
        assert stored["1115962442528"] == 4
...
class TestSafety:
    def test_pause_stops_the_run_before_anything_is_read(self, session, wired, account):
        """
        The one big red button. It must take effect before the vendor
...
class TestAutomaticMode:
    def test_changes_are_sent_and_the_batch_is_recorded(
        ...
        assert set(patched) == {"EXAMPLE-0025543055416", "EXAMPLE-0005319056434"}
    ...
    def test_a_second_run_over_the_same_feed_changes_nothing_more(
        ...
        state = {SKU_FOR["25543055416"]: 131, SKU_FOR["5319056434"]: 2}
        ...
        assert state[SKU_FOR["25543055416"]] == 0
...
class TestScopeDefault:
    """
    A fresh installation must not be able to write to anybody's account.

    Every other test in this file sets ``sku_prefixes_in_scope`` explicitly,
    because they are about the pipeline. This one is about what happens when
    nobody has set it -- which is the state every new deployment starts in, and
    the one where a permissive default would do real damage.
    """

    def test_an_unconfigured_install_has_no_listings_in_scope(self, session):
        """
        The default is an empty prefix list, and an empty list means nothing is
        in scope -- not everything.

        This is the fail-safe direction, and it is easy to get backwards: an
        empty filter conventionally means "no filter, match all". Here that
        reading would mean a fresh install claims ownership of every listing on
        the account, including ones backed by suppliers this system never sees.
        """
        assert settings_store.get(session, "sku_prefixes_in_scope") == []

        index = CatalogIndex(
            [
                ListingEntry(
                    seller_sku="EXAMPLE-0005319056434",
                    sku_prefix="EXAMPLE-",
                    barcode="0005319056434",
                    quantity=3,
                    status="Active",
                    fulfillment_channel="DEFAULT",
                ...
                ListingEntry(
                    seller_sku="ALT-7263956611093",
                    sku_prefix="ALT-",
                    barcode="7263956611093",
                    quantity=9,
                    status="Active",
                    fulfillment_channel="DEFAULT",
                    lead_time_to_ship_days=None,
            ...
            prefixes_in_scope=settings_store.get(session, "sku_prefixes_in_scope"),
        )

        assert index.in_scope_count == 0
        assert index.out_of_scope_count == 2

    def test_a_run_with_no_scope_configured_sends_nothing(self, session, wired, account):
        """
        The end-to-end consequence: with no scope set, a run completes cleanly
        and sends nothing, rather than matching the whole catalogue.
        """
        settings_store.set_value(session, "sku_prefixes_in_scope", [], actor="test")
        settings_store.set_value(session, "sync_mode", SyncMode.AUTOMATIC.value, actor="test")
        session.flush()
        ...
        assert outcome.status is RunStatus.NO_CHANGES
        assert not any(r.method in ("PUT", "PATCH", "POST") and "listings" in str(r.url)
                       for r in calls), "an unconfigured install tried to write"

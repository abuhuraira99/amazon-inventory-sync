...
=====================
This is the code that decides what gets written to a seller account. A bug
here is not a broken page -- it is a product taken off sale that should not
have been, or a quantity promised that cannot be fulfilled.

Both of those cost money and neither raises an exception, so tests are the only
place they can be caught. Every case below encodes a rule whose violation would
do real damage, and the test names say what the damage is.
"""
...
AMS_LISTINGS = [
    listing("EXAMPLE-0007298811035", "0007298811035", 1),
    listing("EXAMPLE-0001749188257", "0001749188257", 1),
    listing("EXAMPLE-4705343071514", "4705343071514", 1),
    listing("EXAMPLE-0932826789213", "0932826789213", 5),
    listing("EXAMPLE-0051449820131", "0051449820131", 0),
]

# Other suppliers on the same account. The system must never touch these.
OTHER_LISTINGS = [
    listing("ALT-7263956611093", "7263956611093", 5),
    listing("LEGACY1-060304883303", "0060304883303", 1),
    listing("SUP6-1234567", "", 3),
]
...
def index():
    """
    A catalogue index scoped to EXAMPLE-.
    ...
    return CatalogIndex(fresh, prefixes_in_scope=["EXAMPLE-"])
...
class TestScope:
    """
    A multi-supplier account, which is the normal case:
        EXAMPLE-   backed by this feed        -> in scope
        ALT-       backed by another feed     -> out
        SUP5-      backed by another feed     -> out
        LEGACY1-   partially matching, ambiguous -> out
    ...
    def test_out_of_scope_sku_is_not_reachable(self, index):
        assert index.by_sku("ALT-7263956611093") is None
        assert index.by_sku("LEGACY1-060304883303") is None
    ...
    def test_out_of_scope_sku_is_still_known_to_exist(self, index):
        ...
        belongs to another supplier" is a completely different message for the operator,
        so both facts are kept.
        """
        assert index.sku_exists_anywhere("ALT-7263956611093") is True
        assert index.sku_exists_anywhere("EXAMPLE-does-not-exist") is False
    ...
    def test_an_empty_scope_touches_nothing(self):
        ...
        assert empty.by_sku("EXAMPLE-0007298811035") is None

    def test_scope_is_case_insensitive(self):
        """A SKU typed by hand in Seller Central may differ in case."""
        idx = CatalogIndex(AMS_LISTINGS, prefixes_in_scope=["example-"])
...
class TestMapping:
    def test_matches_the_vendor_form_of_a_barcode(self, index):
        """
        The vendor sends 7298811035; Amazon holds EXAMPLE-0007298811035.
        This is the padding rule working end to end.
        """
        result = Mapper(index).match("7298811035")
        assert result.matched
        assert result.listing.seller_sku == "EXAMPLE-0007298811035"
        assert result.source is MapSource.BARCODE_EXACT

    def test_matches_an_already_padded_barcode(self, index):
        result = Mapper(index).match("0007298811035")
        assert result.matched

    def test_matches_a_thirteen_digit_barcode_needing_no_padding(self, index):
        result = Mapper(index).match("4705343071514")
        assert result.matched
        assert result.listing.seller_sku == "EXAMPLE-4705343071514"
    ...
    def test_unlisted_barcode_is_held_not_guessed(self, index):
        ...
        assert result.attempted_sku == "EXAMPLE-9999999999999"
    ...
    def test_other_suppliers_barcode_is_reported_as_such(self, index):
        ...
        act on.
        """
        result = Mapper(index).match("7263956611093")
        assert not result.matched
    ...
    def test_manual_override_wins_over_the_rules(self, index):
        ...
        mapper = Mapper(index, overrides={"0009999999999": "EXAMPLE-0007298811035"})
    ...
    def test_override_pointing_at_a_missing_sku_falls_through(self, index):
        ...
        """
        mapper = Mapper(index, overrides={"0007298811035": "EXAMPLE-NOT-REAL"})
        result = mapper.match("7298811035")
    ...
    def test_duplicate_barcode_choice_is_stable(self):
        ...
        dupes = [
            listing("EXAMPLE-0007298811035", "0007298811035", 1),
            listing("EXAMPLE-0007298811035b", "0007298811035", 2, status="Inactive"),
        ]
        idx = CatalogIndex(dupes, prefixes_in_scope=["EXAMPLE-"])
        first = Mapper(idx).match("7298811035").listing.seller_sku
        second = Mapper(idx).match("7298811035").listing.seller_sku
        assert first == second
        # The Active one wins.
        assert first == "EXAMPLE-0007298811035"

    def test_coverage_measurement(self, index):
        """The Stage 0 number, on a small scale."""
        vendor = {"0007298811035", "0001749188257", "4705343071514"}
...
class TestQuantityRules:
    """
    One configured policy, end to end: publish the supplier's figure as-is
    (buffer 0), never above a ceiling (cap 15), off sale only when the
    supplier genuinely has none (out-of-stock at 0).
    """

    @pytest.mark.parametrize(
        ("vendor", "expected"),
        [(0, 0), (1, 1), (5, 5), (15, 15), (16, 15), (27, 15), (4200, 15)],
    )
    def test_a_configured_policy_end_to_end(self, vendor, expected):
        qty, _reason = desired_quantity(vendor, QuantityRules())
        assert qty == expected

    def test_the_cap_is_absolute(self):
        """
        Suppliers report warehouse totals in the thousands. Promising that
        on Amazon carries real risk and no upside, so the cap is absolute --
        applied last, after every other rule.
        """
        qty, reason = desired_quantity(4200, QuantityRules(max_quantity=15))
        assert qty == 15
    ...
    def test_per_format_override(self):
        """
        Feeds carry a product-type column, so "no more than 8 of any one
        product type" must be possible without touching code.
        """
        rules = QuantityRules(max_quantity=15, format_overrides={"BULK": {"max_quantity": 8}})
        assert desired_quantity(20, rules.for_format("BULK"))[0] == 8
        assert desired_quantity(20, rules.for_format("STD"))[0] == 15
...
class TestDecisionEngine:
    def test_no_change_when_already_correct(self, index):
        ...
        match = Mapper(index).match("0932826789213")   # Amazon shows 5
    ...
    def test_raising_a_quantity(self, index):
        engine = DecisionEngine(QuantityRules())
        match = Mapper(index).match("0007298811035")   # Amazon shows 1
    ...
    def test_fba_listings_are_never_touched(self):
        ...
        fba = [listing("EXAMPLE-0007298811035", "0007298811035", 3, channel="AMAZON_NA")]
        idx = CatalogIndex(fba, prefixes_in_scope=["EXAMPLE-"])
        engine = DecisionEngine(QuantityRules())
        decision = engine.decide(Mapper(idx).match("7298811035"), 0)
        assert not decision.should_push
        assert decision.skip_reason is SkipReason.FBA_LISTING

    def test_inactive_listings_are_skipped(self):
        """Accounts accumulate Inactive and Incomplete listings; writing to
        them achieves nothing and clutters the audit trail."""
        inactive = [listing("EXAMPLE-0007298811035", "0007298811035", 1, status="Inactive")]
        idx = CatalogIndex(inactive, prefixes_in_scope=["EXAMPLE-"])
        engine = DecisionEngine(QuantityRules())
        decision = engine.decide(Mapper(idx).match("7298811035"), 9)
        assert not decision.should_push
        assert decision.skip_reason is SkipReason.INACTIVE_LISTING

    def test_blacklisted_sku_is_skipped(self):
        idx = CatalogIndex(AMS_LISTINGS, prefixes_in_scope=["EXAMPLE-"])
        mapper = Mapper(idx, blacklist=["EXAMPLE-0007298811035"])
        engine = DecisionEngine(QuantityRules())
        decision = engine.decide(mapper.match("7298811035"), 9)
        assert not decision.should_push
    ...
    def test_unknown_amazon_quantity_is_not_pushed(self):
        ...
        """
        unknown = [listing("EXAMPLE-0007298811035", "0007298811035", None)]
        idx = CatalogIndex(unknown, prefixes_in_scope=["EXAMPLE-"])
        engine = DecisionEngine(QuantityRules())
        decision = engine.decide(Mapper(idx).match("7298811035"), 5)
        assert not decision.should_push
    ...
    def test_increases_can_be_switched_off(self, index):
        ...
        decision = engine.decide(Mapper(index).match("0007298811035"), 9)
        assert not decision.should_push
    ...
    def test_going_to_zero_still_works_when_increases_are_off(self, index):
        ...
        engine = DecisionEngine(QuantityRules(allow_increases=False))
        decision = engine.decide(Mapper(index).match("0932826789213"), 0)
        assert decision.should_push
        assert decision.direction is Direction.TO_ZERO
    ...
    def test_min_change_never_blocks_a_zero(self, index):
        ...
        decision = engine.decide(Mapper(index).match("0007298811035"), 0)
        assert decision.should_push
        assert decision.direction is Direction.TO_ZERO

    def test_min_change_blocks_small_moves(self, index):
        engine = DecisionEngine(QuantityRules(min_change_to_push=5))
        decision = engine.decide(Mapper(index).match("0932826789213"), 7)  # 5 -> 7
        assert not decision.should_push
...
class TestDroppedProducts:
    """
    Zeroing takes a product off sale. It must never delete anything -- from
    this system or from Amazon -- because a quantity is recoverable and a
    listing, with its reviews and ranking, is not.
    """

    def test_dropped_product_goes_to_zero(self):
        engine = DecisionEngine(QuantityRules())
        decision = engine.decide_dropped(
            listing("EXAMPLE-0007298811035", "0007298811035", 3),
    ...
    def test_already_zero_needs_no_action(self):
        engine = DecisionEngine(QuantityRules())
        assert engine.decide_dropped(
            listing("EXAMPLE-0007298811035", "0007298811035", 0),
    ...
    def test_fba_dropped_product_is_still_not_touched(self):
        engine = DecisionEngine(QuantityRules())
        assert engine.decide_dropped(
            listing("EXAMPLE-0007298811035", "0007298811035", 3, channel="AMAZON_NA"),
...
class TestChangeLimit:
    """
    An account that has never been synced properly starts with a backlog of
    every disagreement at once. The limit spreads that over several runs, so no
    single run looks like a runaway -- to the guardrails or to a human.
    """
...
class TestGuardrails:
    def test_truncated_full_feed_is_refused(self):
        ...
        has sold out of everything", and with an aggressive "zero it straight
        away" rule that would take the whole catalogue off sale.
        """
        result = check_feed_row_count(
            filename="FULL_FEED_100000_20260904.zip",
            row_count=300_000,
    ...
    def test_small_delta_is_fine(self):
        ...
        result = check_feed_row_count(
            filename="DELTA_FEED_100000_20260904_88.zip",
            row_count=41,
            historical_median=1_158_340,
            is_full_feed=False,
            min_percent=50.0,
    ...
    def test_first_full_feed_sets_the_baseline(self):
        result = check_feed_row_count(
            filename="FULL_FEED_100000_20260901.zip",
            row_count=1_150_544,
    ...
    def test_changed_header_is_refused(self):
        ...
        result = check_feed_header(
            ...
            observed="barcode|brand|title|stock|price|format",
            expected="barcode|brand|title|price|stock|format",
            matched=False,
            required=True,
        )
        assert not result.passed
    ...
    def test_normal_zeroing_passes(self):
        """A steady-state day zeroes a small number; this must not trip."""
        decisions = [
            Decision(f"sku-{i}", str(i), 5, 0, 0, Direction.TO_ZERO, "")
            for i in range(196)
        ]
    ...
    def test_the_real_first_run_passes(self):
        """
        A large but legitimate first batch -- most of it quantity
        corrections, a small number going to zero. This must pass, or an
        account with a real backlog could never be brought into sync at all.
        """
        decisions = (
...
class TestNeverSendAPrice:
    """
    Amazon's own flat-file template carries quantity and price in adjacent
    columns, which is exactly how an accidental price write would happen. This
    is the promise that must never break, and it is enforced structurally --
    by the absence of a field -- rather than as a setting somebody could flip.
    """
    ...
    def test_the_real_template_price_attribute_is_refused(self):
        """The exact attribute string Amazon's own template uses."""
        with pytest.raises(PriceFieldRefused):
    ...
    def test_a_feed_document_carries_only_availability(self):
        ...
        doc = json.loads(
            build_feed_document(
                [FeedItem("EXAMPLE-0007298811035", 7)], seller_id="A1EXAMPLESELLER"
            )
        )
...
def test_a_done_feed_can_still_have_rejected_rows():
    """
    A feed routinely reports more rows processed than succeeded, filing the
    difference under "successful with other errors" -- real failures wearing a
    success label. Treating DONE as success hides exactly those.
    """
    import json

    items = [FeedItem(f"EXAMPLE-{i:013d}", 5) for i in range(1, 4)]
    report = json.dumps({
...
def test_an_unreadable_report_admits_ignorance():
    ...
    """
    items = [FeedItem("EXAMPLE-0007298811035", 5)]

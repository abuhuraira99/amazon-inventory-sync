...
def wired_app(monkeypatch):
    ...
    matters: earlier versions of this file created bare API clients for the
    ...
    with factory() as session:
        ...
        session.add(
            VendorProduct(
                barcode="0007298811035", raw_barcode="7298811035",
                brand="CONTOSO", title="GADGET ASSORTED",
                price=9.84, stock=2, product_format="STD",
            )
        )
        session.add(
            Run(
...
def test_settings_can_be_saved(app_client):
    ...
    response = app_client.post(
        ...
        data={
            ...
            "sku_prefixes_in_scope": "EXAMPLE-",
            "feed_delimiter": "|",
            "sku_prefix_for_new": "EXAMPLE-",
...
def test_modern_amzn1_prefixed_client_secret_is_accepted(app_client):
    ...
    have refused a genuine modern secret outright -- the credential could not
...
def test_api_has_no_write_routes():
    ...
    matters, a typed confirmation. An API key that could zero a catalogue
...
# host where the application runs under a service manager -- no console, no log
# file -- that left the operator with no way at all to find out what Amazon had
# actually said.
#
# At the same time the permissions reminder was unconditional. An operator fixes

...
reproduces a real-world feed property or encodes a rule whose violation was
*measured* to break the system:

  * getting the padding wrong silently loses every zero-padded listing
...
from tests.conftest import IN_SCOPE_SKUS, REAL_MALFORMED_SKUS
...
class TestPaddingIsTheWholeGame:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("7298811035", "EXAMPLE-0007298811035"),   # 10 digits -> 3 zeros
            ("1749188257", "EXAMPLE-0001749188257"),
            ("51449820131", "EXAMPLE-0051449820131"),  # 11 digits -> 2 zeros
            ("932826789213", "EXAMPLE-0932826789213"), # 12 digits -> 1 zero
            ("4705343071514", "EXAMPLE-4705343071514"),# 13 digits -> unchanged
        ],
    )
    def test_builds_the_sku_that_actually_exists(self, vendor_sends, amazon_has):
        assert build_sku("EXAMPLE-", vendor_sends) == amazon_has
    ...
    def test_naive_concatenation_would_be_wrong(self):
        ...
        """
        vendor = "7298811035"
        naive = "EXAMPLE-" + vendor
        correct = build_sku("EXAMPLE-", vendor)
        assert naive == "EXAMPLE-7298811035"
        assert correct == "EXAMPLE-0007298811035"
        assert naive != correct

    def test_canonical_is_always_thirteen_wide(self):
        for raw in ("1", "12345678", "7298811035", "932826789213", "4705343071514"):
            bc = normalise(raw)
            if bc.usable:
    ...
    def test_matches_the_clients_spreadsheet_formula(self):
        """
        The rule is: pad the barcode to exactly 13 digits, then prepend the prefix.
        This asserts we produce byte-identical output for that formula.
        """
        for raw in ("30322617303", "5319056434", "249094965103"):
...
class TestRejectsJunk:
    ...
    def test_no_sku_is_built_for_junk(self, junk):
        assert build_sku("EXAMPLE-", junk) is None
...
class TestHandlesMessyInput:
    def test_strips_separators_and_whitespace(self):
        assert normalise("  535921-051355 ").canonical == "0535921051355"
        assert normalise("0 240825 208315").canonical == "0240825208315"

    def test_handles_spreadsheet_floats(self):
        """
        Spreadsheets store barcodes as floats in scientific notation
        (column G contains values like 6.5103912346E10). str() on those gives
        '65103912346.0' or worse, so they are converted through int().
        """
        assert normalise(6.5103912346e10).canonical == "0065103912346"
        assert normalise(65103912346).canonical == "0065103912346"

    def test_integer_input(self):
        assert normalise(7298811035).canonical == "0007298811035"
    ...
    def test_raw_is_preserved_for_audit(self):
        ...
        """
        bc = normalise("  7298811035 ")
        assert bc.raw == "  7298811035 "     # untouched, for the audit trail
        assert bc.digits == "7298811035"     # cleaned
        assert bc.canonical == "0007298811035"  # padded, used to build the SKU
...
class TestCheckDigit:
    ...
    @pytest.mark.parametrize(
        "code",
        [
            "005319056434",   # a 12-digit UPC-A, valid once the zero is restored
            "002383326746",   # Smart Travels Europe
            "1115962442528",  # a sample product, real EAN-13
        ],
    )
    ...
    def test_stripped_form_does_not_validate(self):
        ...
        assert is_valid_gtin("5319056434") is False   # as the vendor sends it
        assert is_valid_gtin("005319056434") is True  # once restored

    def test_check_digit_computation(self):
        assert gtin_check_digit("00531905643") == "4"
        assert gtin_check_digit("111596244252") == "8"
...
class TestMatchCandidates:
    def test_canonical_form_is_tried_first(self):
        cands = match_candidates("5319056434")
        assert cands[0] == "0005319056434"

    def test_includes_twelve_and_fourteen_digit_forms(self):
        cands = match_candidates("5319056434")
        assert "005319056434" in cands   # UPC-A width
        assert "00005319056434" in cands  # GTIN-14 width

    def test_includes_the_raw_form(self):
        assert "5319056434" in match_candidates("5319056434")

    def test_no_duplicates_and_order_preserved(self):
        cands = match_candidates("4705343071514")  # already 13 wide
        assert len(cands) == len(set(cands))
        assert cands[0] == "4705343071514"

    def test_candidate_skus_all_carry_the_prefix(self):
        for s in candidate_skus("EXAMPLE-", "5319056434"):
...
class TestSplitSku:
    ...
    @pytest.mark.parametrize(
        ("sku", "prefix", "digits"),
        [
            ("EXAMPLE-0007298811035", "EXAMPLE-", "0007298811035"),
            ("ALT-7263956611093", "ALT-", "7263956611093"),
            ("0073555453232", "", "0073555453232"),           # bare barcode SKU
            ("LEGACY1-060304883303", "LEGACY1-", "060304883303"),
            ("SUP4-1234567890123-V.G", "SUP4-", "1234567890123"),  # condition suffix
            ("SUP2-1234567890123-LN", "SUP2-", "1234567890123"),
        ],
    )
    ...
    def test_suffixed_sku_with_trailing_letter(self):
        # some listings carry a condition suffix: ALT-<13 digits>X
        p, d = split_sku("ALT-7263956611093X")
        assert p == "ALT-"
        assert d == "7263956611093"

    def test_prefix_helper(self):
        assert sku_prefix("EXAMPLE-0007298811035") == "EXAMPLE-"
        assert sku_prefix("ALT-7263956611093") == "ALT-"
    ...
    def test_round_trip_through_barcode(self):
        for sku in IN_SCOPE_SKUS:
            bc = barcode_from_sku(sku)
            assert bc.usable
            assert build_sku("EXAMPLE-", bc.canonical) == sku

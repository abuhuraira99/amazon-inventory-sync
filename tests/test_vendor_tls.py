...
====================
An application can talk to one TLS service perfectly and be unable to talk to
another on the same machine, in the same process, with nothing wrong at either
end.

The cause is having two trust stores without noticing: ``httpx`` builds its
default context from ``certifi``, while ``ftplib`` uses the operating system's
store. On Windows that OS store is not fixed -- it ships with a small set of
roots and fetches the rest on demand, for the *system* TLS stack rather than
for Python's. So a server whose certificate is valid, whose chain is complete
and whose credentials are right is rejected with CERTIFICATE_VERIFY_FAILED,
purely because the machine has never fetched the root it chains to.
...
and that must never be how this problem is solved on a system holding
credentials to a seller account.
"""
...
def test_certificate_verification_stays_switched_on() -> None:
    ...
    read access to a supply feed. A context that skips either check
...
def test_a_dropped_connection_becomes_a_vendor_error(tmp_path) -> None:
    """
    A dropped control connection, asserted at the exact boundary.

...
The specific failures guarded here are the ones that would be *plausible*:
  ...
  * something that is not a feed at all -- a nested archive, a database backup
    -- dropped in the folder by mistake, expanding until the disk is full
    and the sync stops.
...
FEED = "barcode|brand|title|price|stock|format\n1115962442528|NORTHWIND|WIDGET STANDARD|12.12|3|STD\n"
...
class TestDamagedArchives:
    def test_a_truncated_download_is_refused(self, tmp_path):
        ...
        """
        good = _zip(tmp_path / "FULL_FEED_100000_20260903.zip", "feed.txt", FEED * 200)
        raw = good.read_bytes()
...
class TestHeaderGate:
    def test_a_changed_header_stops_the_parse(self, tmp_path):
        ...
        """
        swapped = "barcode|brand|title|stock|price|format\n1115962442528|A|B|3|12.12|STD\n"
...
class TestControlCharactersNeverReachTheDatabase:
    def test_a_nul_byte_is_removed_from_a_title(self) -> None:
        """The exact byte, in the exact field, that kills a catalogue load."""
        from app.vendor.parser import _clean

        cleaned = _clean("WIDGET\x00 & CO")

        assert "\x00" not in cleaned
        assert cleaned == "WIDGET & CO"

    def test_the_other_control_characters_go_too(self) -> None:
        """
        Only NUL is rejected by PostgreSQL; the rest are still junk.

        They are meaningless inside an brand or a title and they corrupt any
    ...
    def test_a_row_carrying_a_nul_byte_still_parses(self, tmp_path) -> None:
        ...
        body = (
            ...
            "1115962442528|NORTH\x00WIND|WIDGET\x00 STANDARD|12.12|3|STD\n"
        )
        ...
        assert len(rows) == 1
        assert "\x00" not in rows[0].brand
        assert "\x00" not in rows[0].title
        assert rows[0].brand == "NORTHWIND"

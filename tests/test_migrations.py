...
def test_migrated_keys_and_indexes_match_the_models(migrated_schema, model_schema):
    ...
    matching a whole catalogue against a million-row feed finish in seconds
    rather than minutes, and a missing index shows up only as a system that has
    quietly become too slow to keep up with its own schedule.
    """
    problems: list[str] = []
    for table in sorted(set(model_schema) & set(migrated_schema)):
...
def test_the_alembic_environment_loads_dotenv():
    ...
    there and ONLY there. Run ``alembic upgrade head`` directly on a host --
    which is what the non-container deployment path does, and what
    ``scripts/deploy.ps1`` does -- and DATABASE_URL is simply absent, because it
    lives in .env and nothing has loaded it. First-time setup cannot create its
    tables, and every later release carrying a migration stops at the same
    line.
    ...
    assert "load_dotenv" in source, (
        ...
        "deployment (scripts/deploy.ps1) has nothing else to read DATABASE_URL "
        "from, and every migration fails with 'DATABASE_URL is not set'."
    )

...
field, an operator edits it, the save succeeds, the page redisplays the new
...
* ``sync_offset_minutes`` -- the rescheduler compared only the interval, so
  ...
  an operator as "you have hard coded the interval". It is not
...
class TestTheTimezoneIsValidated:
    ...
    @pytest.mark.parametrize(
        "name",
        ["Europe/Berlin", "America/New_York", "Europe/London", "UTC"],
    )
    ...
    def test_the_refusal_says_what_a_good_value_looks_like(self) -> None:
        """
        An error nobody can act on is barely better than no error. This is
        typed by hand -- the field has no dropdown -- so the message has to
        carry a worked example, not just a complaint.
        """
...
def test_every_setting_is_read_by_something() -> None:
    ...
    version -- a field offered to an operator that no code consults at all. That

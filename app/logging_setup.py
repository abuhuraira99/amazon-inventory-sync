...
def configure() -> None:
    ...
    # `docker compose logs` shows everything. That is the happy case, and it is
    # easy to assume it is the only case.
    #
    # It is not. Run under a service manager -- a Windows Scheduled Task, a
    # bare systemd unit without journal capture, a cron entry -- stdout goes
    # nowhere at all. Every diagnostic the application produces is discarded,
    # silently, and the first time anybody notices is when something fails and
    # there is no account of it anywhere: nothing on the dashboard, nothing in
    # a file, nothing to read. Being unable to answer "what did it do?" is
    # worse than
    ...
    if file_handler_error is not None:
        logging.getLogger(__name__).warning(
            "could not open the log file (%s); logging to stdout only, which "
            "under a service manager may mean the log is discarded entirely",
            file_handler_error,
        )

"""
The settings screen: everything an operator can change without a developer.
...
#: Section order and headings. Written for the operator, so the page reads as a
...
async def save_settings(
    ...
    fails, so the operator sees every problem at once rather than fixing them one
    ...
    # indistinguishable from a setting that does not work, and the natural
    # suspicion falls on the VALUE rather than on the plumbing that delivers
    # it. Applied here, the next-run time on the page is correct as
...
async def save_credential(
    ...
    practical: the operator types their own values here, over HTTPS, and they are
...
async def test_amazon(
    ...
    before anyone is ready to let the system write anything.
    """

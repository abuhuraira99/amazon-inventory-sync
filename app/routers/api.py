...
Two things, and deliberately not more:
  ...
  2. **External monitoring.** Being able to ask "is the inventory sync
     healthy" from somewhere else -- an uptime checker, a status board, another
     internal system -- is worth the twenty lines it costs.
...
it matters, a typed confirmation. An API key that could zero a catalogue
...
async def coverage(
    ...
    This is the first question worth answering about any installation, before
    a single write is enabled, and it stays worth re-asking: a sudden drop
    means the supplier changed their barcode format or somebody renamed SKUs
    in Seller Central.
    """
    cfg = settings_store.get_all(session)
    prefixes = [p.upper() for p in cfg["sku_prefixes_in_scope"]]

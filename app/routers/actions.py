...
sends thousands of quantity changes to a real Amazon account. They are
POST-only, and
...
async def batch_detail(
    ...
    Every item is shown with the reason in words. An approval step that does
    not let the approver see what they are approving is theatre rather than a
    control, and a batch is exactly the size where that is tempting to skip.
    """
...
async def undo_preview(
    ...
    An undo is itself a write to the account, and deserves the same review as

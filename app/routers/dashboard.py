"""
The pages an operator actually looks at.
...
def _measure_drift(session: Session, prefixes: list[str], cap: int) -> dict:
    ...
    Done as one SQL statement rather than in Python: it joins every in-scope
    listing against every vendor row, and pulling either side
...
async def products(
    ...
    if q:
        ...
        if candidates:
            ...
            like = f"%{term}%"
            query = query.where(
                (VendorProduct.title.ilike(like)) | (VendorProduct.brand.ilike(like))
            )
...
async def unmatched(
    ...
    an operator can use directly. Nothing here has been sent to Amazon -- that is
...
async def reports(
    ...
    In Stock and New Products are the two most useful day to day, and the

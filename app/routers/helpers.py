...
human ("2m 14s", never "134.2"); a timestamp is always shown in the
configured timezone, because an operator should not have to convert UTC in
their head at 6am.
"""
...
def fmt_int(value: Any) -> str:
    """``12345`` -> ``"12,345"``. Blank for None, never "None"."""
...
def fmt_datetime(value: Any, tz_name: str | None = None) -> str:
    """
    A timestamp in the configured timezone.
    ...
    default city is a lie waiting to happen: a caller who forgot the argument
    would render times for a zone nobody chose -- correctly formatted and
    confidently wrong, with nothing on screen to give it away.
    Falling back to UTC and SAYING so is worse-looking

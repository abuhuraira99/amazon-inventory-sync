...
====================
Nobody but the account owner needs to handle these. Once the server is up, the
operator types their own credentials into the dashboard over HTTPS and this
module encrypts them immediately. Afterwards the interface can only show the
last four characters -- there is no code path that returns a stored secret to a
browser.

That matters for more than tidiness: it means a developer can build, test and
support the system without ever holding a credential that could reach a live
account. The whole test suite runs against sample files and fakes.

WHY BOTH ENVIRONMENT AND DATABASE
=================================
Values may come from either. The database wins when both are present, because
that is where an operator's own entry lands, and a deliberate entry should
beat a leftover value in a ``.env`` file. Environment variables remain useful for
...
SECRET_SPECS: tuple[SecretSpec, ...] = (
    SecretSpec(
        ...
        help_text=(
            "The password for the vendor's file server. For a vendor offering "
            "explicit FTP over TLS, this is the password that goes with the "
            "username on port 21."
        ),
...
def get_secret(session: Session, key: str) -> str:
    ...
    **The only function in the codebase that returns a decrypted secret.**
    ...
    a clear, actionable message for the operator, which is much more useful
    than a stack trace.
    """
    spec = SPEC_BY_KEY.get(key)
    if spec is None:
...
def missing_required(session: Session) -> list[str]:
    """
    Which required credentials are absent, described for the operator.
...
def _sanity_check(spec: SecretSpec, value: str) -> None:
    ...
    if spec.key == "lwa_client_secret":
        ...
        #
        # A tempting version of this check rejects anything starting "amzn1."
        # on the theory that only IDs carry that prefix. That is wrong, and it
        # refuses a genuine modern secret outright -- making the credential
        # impossible to enter at all. Amazon's own LWA credentials

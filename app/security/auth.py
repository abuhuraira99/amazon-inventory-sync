...
===========================
One shared administrator account, seeded at first start. No user management
screens, no invitations, no role editor.

This is a deployment for one or two people operating one seller account, and
a user-management system for that is a large surface area -- invitations,
password resets, permission edits -- defending against a threat that does not
exist. Every one of those screens is also somewhere a session bug can live.
...
account that takes real orders. So:
...
#: guessing impractical, short enough not to lock an operator out for a day
#: because of a typo.
...
def totp_provisioning_uri(secret: str, email: str) -> str:
    ...
    """
    return pyotp.TOTP(secret).provisioning_uri(name=email, issuer_name="Amazon Inventory Sync")

...
places on purpose:
  ...
  * **Business behaviour** -> the ``settings`` table, read through
    ...
    alert recipients. An operator changes these from the dashboard without a
    deploy, which was an explicit requirement.

If you are about to add a knob here, ask whether an operator might ever want to
...
class Settings(BaseSettings):
    ...
    # ------------------------------------------------------------------ app
    app_name: str = "Amazon Inventory Sync"
    ...
    #: US marketplace. This is Amazon's published constant for it, not an account
#: identifier; see the marketplace table in Amazon's SP-API documentation for
#: the other regions. Cross-checked against a real Price & Quantity
    ...
    # is the recommended route: the operator types them in once, the system
        ...
        if self.is_production:
            ...
            if self.database_url.startswith("sqlite"):
                ...
                problems.append(
                    ...
                    "    2. a full feed upserts millions of rows while the\n"

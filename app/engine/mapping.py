...
===========================
The SKU for a supplier barcode is usually built by a rule as simple as::

    sku = prefix + zero_pad(barcode, 13)

That rule can be perfectly correct and still not answer the question that
matters, because "the SKU is correctly formed" and "the SKU exists on the
account" are two different claims, and only Amazon can settle the second.

This matters more than it sounds. An update addressed to a SKU that does not
exist is **accepted and discarded** -- no error, no rejection, nothing in the
response to distinguish it from a real write. So a matching bug produces no
symptom at all. It just leaves part of the catalogue permanently unsynced.

Everything in this module exists to replace an assumption ("the formula is
right, so the SKU must be there") with evidence ("this SKU was in the
catalogue Amazon reported to us").

SCOPE BY SKU PREFIX
===================
Sellers who buy from several suppliers usually encode the source in the SKU
prefix. That makes the prefix a cheap and reliable scope filter -- and, more
importantly, a safety boundary.

An illustrative census of an account, to show the shape of the problem
(these are example figures, not measurements)::

    prefix        listings   present in this supplier's feed
    EXAMPLE-        45,000          99%      <- the supplier we sync
    ALT-            22,000           0%      <- a different supplier
    SUP5-            4,000           0%
    LEGACY1-         2,000          85%      <- ambiguous, out of scope

Two conclusions drive the design:

  1. A prefix that is almost entirely backed by one supplier's feed is a
     trustworthy scope filter. Pushing a quantity derived from this feed onto
     an ``ALT-`` listing would overwrite a correct quantity, sourced from a
     feed this system never sees, with an unrelated number.

  2. **Partial matches are the dangerous case.** A prefix that matches only
     70-85% looks like it belongs to this supplier but is not certain. Under a
     "missing from the full feed means set to zero" rule, the 15-30% that are
     absent would be switched off -- possibly wrongly. Prefixes like that are
     deliberately excluded rather than half-included: a scope filter that is
     mostly right is worse than one that is narrow, because the failures are
     silent and land on real listings.

The prefixes in scope are a setting, not a constant. See
...
class CatalogIndex:
    ...
    Why in memory: the decision engine looks up every feed barcode against
    every in-scope listing. At feed sizes of a million rows, doing that with
    one database query per barcode would mean a million round trips -- the
    dominant cost of the whole run. Indexing once turns each lookup into a
    dictionary hit. The index holds only the fields matching needs, so it stays
    in the tens of megabytes even for a large catalogue.
    ...
    def __init__(
        ...
        *,
        ...
        #: "this SKU does not exist" from "it exists but belongs to another supplier",
...
class Mapper:
    ...
    def _pick(self, candidates: list[ListingEntry]) -> ListingEntry:
        ...
        A barcode is *supposed* to identify exactly one product, and feeds
        generally honour that. Amazon accounts, however, do accumulate
        duplicate listings for the same product over time. Rather than assume
        the clean case, we pick deterministically: an Active, merchant-fulfilled, non-blacklisted
...
class CoverageReport:
    ...
    This is deliberately useful on its own. It answers "how much of this
    catalogue could we sync, and what would we miss?" without sending
    anything -- which is the question worth answering *before* deciding
    whether to turn writes on at all.
    """
    ...
    def listing_coverage_percent(self) -> float:
        ...
        This is the direction that matters. The reverse figure -- what share
        of the supplier's catalogue is listed -- is naturally low and carries
        no signal, because a seller lists a deliberately chosen subset of what
        a distributor offers. A low number there is a business choice; a low

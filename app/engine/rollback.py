...
==================================================
This system writes to an account that takes real orders.
...
def _believed_current_quantity(item: PushItem, listing: AmazonListing) -> int | None:
    ...
    of a large batch (see VERIFY_SAMPLE_THRESHOLD in app/engine/pusher.py).

    So for the great majority of items in a sampled batch, ``listing.quantity`` still holds
    ...
    considered safe to point at an account that takes real orders at all.
    ...
    written twice. Failing to restore one leaves the seller offering stock that

...
here controls. The realistic disasters, in order of how much they would cost:
  ...
  1. **A truncated vendor file.** A download cut short reads as "the vendor has
     sold out of everything". Without a brake, the system would take the
     entire catalogue off sale, and nothing would look wrong while it
     happened -- every individual write would succeed. Two independent
     defences catch this: the archive CRC check in :mod:`app.vendor.parser`
     and the zeroing limit here.
...
class GuardrailResult:
    ...
    passed: bool
    severity: Severity
    #: Written for the operator, not for a developer. Goes straight into the
    #: dashboard and the alert email, so it has to explain itself to the person
...
def check_feed_row_count(
    *,
    ...
    THE most important check in the system, because of how a truncated
    download presents itself: as a perfectly valid feed that simply has fewer
    rows. Every product absent from the short file looks discontinued, and an
    aggressive "missing means zero" setting turns that into the whole catalogue
    going off sale.

    The comparison is against the MEDIAN of previous feeds rather than the
    last one, so a single anomalous feed cannot drag the baseline with it --
    which is the failure this check exists to catch in the first place.

    Only applied to full feeds. Delta files are legitimately tiny, so a size
    floor there would be meaningless.
    """
    if not is_full_feed:
...
def check_rejected_row_ratio(
    *, filename: str, rejected: int, total: int, max_percent: float = 5.0
    ...
    A warning rather than a halt. Every real feed carries a small tail of
    unreadable rows -- truncated values, stray characters, data-entry
    mistakes -- and losing a whole cycle's stock updates because a few more
    than usual arrived would be a bad trade. But a sudden jump means the
    supplier has changed something, and that is worth a human's attention.
    """
    if total == 0:
...
def check_percent_changed(
    *, proposed: int, in_scope_total: int, max_percent: float
    ...
    The blunt instrument. It does not know *why* a run wants to change a large
    share of the catalogue, only that steady-state runs change a small one, and
    that a sudden jump deserves a human's attention before it reaches the
    account.
...
def check_zeroing_limit(*, decisions: list[Decision], max_zeroing: int) -> GuardrailResult:
    ...
    Calibration is the difficulty here. In steady state the number of products
    going to zero on any given day is small -- a few hundred at most on a large
    catalogue -- while a truncated feed would zero everything. The default
    leaves a wide margin over normal behaviour while still being orders of
    magnitude below a catalogue-wide wipe, and it is a setting because the
    right value depends on the catalogue.
    """
...
def check_no_price_in_decisions(*, decisions: list[Decision]) -> GuardrailResult:
    ...
    live listing. Cheap insurance on the promise that matters most, and the
    reason it is expressed as a type rather than as a rule somebody has to
    remember.
    """
    suspicious = [
        d.seller_sku

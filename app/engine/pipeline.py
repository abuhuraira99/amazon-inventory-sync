...
================
    ...
    6  DECIDE     apply the quantity rules, compare against Amazon
...
from app.engine.report_builder import (
    ReportRow,
    prune_empty_report_directories,
    write_all_reports,
)
...
#: for million-row full feeds: large enough that the round trips do not
...
def close_interrupted_runs(session: Session) -> int:
    ...
    supervisor or service manager stopping the job. The row keeps the status it
    was created with and stays RUNNING for ever.
...
def _recover_session(session: Session, run: Run, run_id: int) -> Run:
    ...
    cause was buried under a rollback error that says nothing about it -- so
    the one piece of information worth having, the original exception, is
    exactly the piece that gets lost.
...
def execute_run(
    ...
    *,
    ...
    try:
        ...
        # counter and nothing else. So a full feed proposing far more changes
        # than the per-run limit sent the limit and let the remainder evaporate:
        # the delta runs that followed could not see them, because a delta only
        # mentions what changed, so they waited for the NEXT full feed a day
        # later. The backlog therefore drained at one change-limit per DAY
        # rather than per run, which is arbitrarily slower than the settings
        # page implies. The visible symptom is an operator clearing it by hand
        # with "Reconcile everything".
        #
        # Reconciling every run costs one pass over the in-scope listings,
...
def _housekeeping(session: Session, run: Run, cfg: dict, outcome: RunOutcome) -> None:
    ...
    removed_snapshots = 0
    # `backups_dir`, NOT `data_dir / "snapshots"`.
    #
    # The Amazon layer is handed `settings.backups_dir` and writes
    # `catalog-<stamp>.tsv` there on every refresh. This looked in
    # `data/snapshots`, a directory that has never existed anywhere in this
    # application -- so `is_dir()` was false on every single run and the
    # retention setting deleted nothing, ever.
    #
    # It survived because the test written for it used the wrong directory too:
    # the test and the bug agreed, and both disagreed with the running system.
    # Harmless while the refresh was nightly at ~17 MB a day. Once the refresh
    # became hourly that is ~400 MB a day, forever, on a 30 GB disk -- and a
    # full disk is the failure this whole function exists to prevent.
    snapshot_dir = app_settings.backups_dir
    ...
    # After the files, the folders they lived in. One empty `run-N` directory
    # was left behind per run, for ever -- 24 a day on an hourly schedule.
    removed_dirs = prune_empty_report_directories(app_settings.reports_dir)

    if removed_feeds or removed_reports or removed_snapshots or removed_dirs:
        log.info(
            "housekeeping: removed %d feed archive(s), %d report(s), %d snapshot(s), "
            "%d empty folder(s), freeing %.1f MB",
            removed_feeds, removed_reports, removed_snapshots, removed_dirs,
            freed / 1024**2,
        )

    # ---- vendor change history -------------------------------------------
    # The largest table in the database, and the only unbounded one left. A row
    # is written for every stock OR price change, so the first full feed writes
    # one per product in the supplier's catalogue. Price changes are recorded
    # even though this system never acts on a price, because the generated
    # reports are built from them.
    #
    ...
    # ---- the per-product change log --------------------------------------
    # One row per quantity this system changes, each holding the value Amazon
    # had before it. These are the rows Undo reads, which makes them the most
    # valuable in the database -- and among the fastest growing, since a busy
    # installation writes tens of thousands a day for as long as it runs. Left
    # alone it reaches millions of rows, and a full disk is precisely the
    # failure this function exists to prevent.
    #
    # TWO CONDITIONS, AND THE SECOND IS THE IMPORTANT ONE
    # ===================================================
    # 1. The batch is older than the retention window.
    # 2. NOTHING in that batch is still PENDING or ACCEPTED.
    #
    # The second is not a nicety. PENDING means never sent -- a batch still
    # waiting for approval is entirely PENDING, and deleting its rows would
    # empty an approval an operator was about to give. ACCEPTED means sent but
    # not yet confirmed, which is exactly what the catalogue refresh comes back
    # for. Age alone would eventually delete both. So the age test decides
    # WHICH batches are candidates and this test decides whether each one has
    # genuinely finished, and a batch that never finishes is simply never
    # pruned.
    #
    # WHAT SURVIVES
    # =============
    # The push_batches row, with its totals -- item count, how many went to
    # zero, how many were confirmed. So the run history still shows what
    # happened and how big it was; only the per-SKU detail goes, the same trade
    # already made for report files.
    #
    # WHAT UNDO DOES AFTERWARDS
    # =========================
    # Refuses, out loud. `execute_rollback` raises "There is nothing to undo"
    # on an empty plan, so a pruned batch cannot silently appear to succeed --
    # the failure this project has already had once with rollback, and the
    # reason that guard exists. Undoing a three-month-old quantity would put a
    # three-month-old number over today's correct one, so refusing is the right
    # answer regardless of retention.
    keep_items = int(cfg.get("keep_push_items_days", 90))
    items_cutoff = now - timedelta(days=keep_items)

    unfinished = (
        select(PushItem.batch_id)
        .where(PushItem.result.in_([ItemResult.PENDING, ItemResult.ACCEPTED]))
        .distinct()
    )
    prunable = (
        select(PushBatch.id)
        .where(PushBatch.created_at < items_cutoff, PushBatch.id.notin_(unfinished))
    )
    deleted_items = cast(
        "CursorResult[Any]",
        session.execute(delete(PushItem).where(PushItem.batch_id.in_(prunable))),
    ).rowcount
    if deleted_items:
        log.info(
            "housekeeping: removed %s per-product change row(s) from batches older "
            "than %d days; the batches and their totals remain",
            f"{deleted_items:,}", keep_items,
        )
    ...
    if free_gb < threshold_gb:
        message = (
            ...
            "  4. Give the machine a larger disk."
        )
...
def _recover_interrupted_batches(
    ...
    host reboot part-way through a send leaves a batch marked SENDING with some
...
def _ingest(
    ...
    changed rather than the whole catalogue.
    """
    ...
    with connect(creds) as vendor:
        ...
        for entry in remote:
            ...
            if only_today and not fname.is_from_today(parsed, tz):
                # The "only today's files" rule. Exact, because the date is in
                # the filename rather than inferred from a timestamp.
                run.files_skipped += 1
                continue
    ...
    # second for a delta, but many minutes for a million-row full feed. A
    # server closes a connection idle that long -- correctly; it is
...
def _parse_and_store(
    ...
    *,
    ...
    Streams the file, so a million-row full feed never exists in memory
    all at once.
    ...
    for row, reject, stats in iter_rows(
        ...
        pending.append(
            (row.barcode.canonical, row.barcode.raw, row.brand, row.title,
...
def _upsert(
    ...
    One SELECT for the whole block, then per-row work in memory. At full-feed
    volumes, one query per row would mean millions of round trips -- the
    ...
    for barcode, raw, brand, title, price, stock, fmt, checksum_ok in rows:
        current = existing.get(barcode)

        if current is None:
            session.add(
                VendorProduct(
                    barcode=barcode, raw_barcode=raw[:40], brand=brand, title=title,
        ...
        if stock_changed or price_changed:
            current.brand = brand or current.brand
...
def _mark_missing_from_full_feed(session: Session, record: FeedFile) -> None:
    ...
    Done in SQL because it touches every vendor row, and pulling millions of
    rows into Python to increment an integer would be absurd.
    """
    session.flush()
...
def _record_unmapped(session: Session, barcode: str, match, stock: int) -> None:
    ...
    Only in-stock products are recorded, and that filter is what makes this
    list usable. A seller lists a deliberately chosen subset of a distributor's
    catalogue, so the great majority of feed rows are unmatched by design.
    Recording all of them would bury the few that are actionable. A product the
    supplier HAS in stock and the seller does not list is either a missed
    opportunity or a broken mapping; one the supplier does not stock is
    neither.
    """
    ...
    if row is None:
        session.add(
            UnmappedBarcode(
                ...
                brand=(product.brand if product else "")[:400],
...
def _write_reports(
    ...
    delta run too. Delta reports are written as separate files so a partial
    picture is never mistaken for the daily one.
    """
    if not files:
        return
    ...
    data = {
        ...
        # Streamed rather than materialised: this is the whole in-stock
        # catalogue, and holding it in memory to write it out would double the
        # peak footprint for no benefit.

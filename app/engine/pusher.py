...
2. **Reads Amazon back afterwards.** A flat-file upload that names a SKU which
   ...
   still contain rejected rows -- "the feed was processed" and "your changes
   were applied" are different claims, and only the second one matters. So
   after every push the affected SKUs are re-read and compared. What did not
   stick is recorded and retried, rather than assumed.
...
#: a change that was fine all along -- which is how an operator learns to
#: ignore a real alarm.
VERIFY_CONFIRM_DEADLINE = timedelta(hours=6)

#: Verifying a large batch item by item costs one GET per SKU, which at
#: thousands of SKUs is slower than the sync interval itself. Above this
#: threshold a random sample is verified and the rest is left to the next
#: catalogue refresh -- cheap, and honest as long as the dashboard says so.
VERIFY_SAMPLE_THRESHOLD = 500
VERIFY_SAMPLE_SIZE = 100

#: Most rows one catalogue confirmation pass will settle at a time.
#:
#: Generous on purpose: many runs' worth at any sane change limit, so normal
#: operation is nowhere near it. It exists only so that a confirmation backlog
#: can never turn into a memory failure on a small machine.
CONFIRM_FROM_CATALOGUE_LIMIT = 50_000
...
def send_batch(
    ...
    *,
    ...
    Per-SKU failures are recorded and the batch continues -- one broken
    listing must not stop every good one behind it. Systemic failures
    (authentication,
...
def verify_batch(
    ...
    *,
    ...
    This is the step most inventory tools skip, and skipping it is how updates
    get lost invisibly. Amazon accepting a request is not the same as Amazon
    applying it, and nothing in a successful response distinguishes the two.

    For batches above ``sample_threshold`` a random sample is verified rather
    than every SKU: at thousands of SKUs, individual GET requests take longer
    than the sync interval, for information the next catalogue refresh provides
    for free. The dashboard
    ...
    # confirmed kept reporting a handful, which is what makes a large run look
    # permanently stuck when it is in fact progressing.
    session.flush()
...
def confirm_from_catalogue(session: Session, quantities: dict[str, int]) -> tuple[int, int]:
    ...
    small batch and hopeless for a large one: at a self-imposed 2 requests per
    second, a few thousand GETs take longer than the sync interval. So batches
    above
    ...
    The trap is that a promise like that is easy to write in a docstring and
    easy to never implement -- and its absence produces no error. Every
    unsampled item simply stays ACCEPTED for ever. A large run then reports a
    handful confirmed and never moves, while the quantities on Amazon have in
    fact been correct all along, and "Refresh catalogue" appears to do nothing
    because for these rows it did nothing.

    This function is that fallback, actually built.
    ...
    requests at all** -- the data is already in memory. Reading those SKUs
    back individually to learn the same thing would be pure waste.
    ...
    #
    # In normal running this holds one run's worth, cleared by the next
    # refresh. But an item only leaves ACCEPTED when something confirms it, and
    # retention deliberately never prunes an unconfirmed row, so if
    # confirmation ever stopped working these would accumulate with nothing to
    # stop them -- and this query would load every one into memory, once an
    # hour, on whatever machine it happens to be. The cap makes that a slow
    # queue instead of a
    # memory failure; oldest first means the backlog still drains in order
    # rather than the same rows being retried while older ones starve.
    open_items: list[PushItem] = list(
        ...
        .order_by(PushItem.batch_id, PushItem.id)
        .limit(CONFIRM_FROM_CATALOGUE_LIMIT)
        .all()
    )
    if not open_items:
        return (0, 0)

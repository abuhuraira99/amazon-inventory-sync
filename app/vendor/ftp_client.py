...
THE SHAPE OF A TYPICAL VENDOR SETUP
===================================
All of this is configuration, not code -- see ``.env.example``:
...
====================
A supplier's file server is usually far less defended than a commercial API,
which is a reason to be more careful rather than less. One connection is held
for one cycle rather than reconnecting per file:
...
def _tls_context() -> ssl.SSLContext:
    ...
    the machine's certificate store -- and that store is not a fixed thing. An
    installation ships with a small set of roots and fetches the rest on
    demand, and that on-demand fetch happens for the *system* TLS stack, not
    for Python's. So a browser on the same machine can load a site happily
    while Python refuses it.

    The failure this produces is badly misleading. A server presenting a
    perfectly valid certificate with a complete chain is rejected with
    CERTIFICATE_VERIFY_FAILED / "unable to get local issuer certificate",
    purely because the chain terminates in a root the machine has never
    fetched. Recently-issued roots are the usual culprit. Nothing is wrong with
    the server, the credentials or the code, so every obvious place to look is
    the wrong one.
...
class _FtpsClient(VendorClient):
    """Explicit FTP over TLS, the common case for supplier file servers."""
    ...
    def list_entries(self) -> list[tuple[str, str]]:
        ...
        there. A credentials sheet saying the path is "/" while the files live
        one folder down is an extremely common first-run problem, and the
        difference between a dead end and an obvious answer is simply showing
        the operator what the server actually contains.

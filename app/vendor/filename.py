...
WHY THE FILENAME IS THE BEST SOURCE OF TRUTH
============================================
"Only process today's files" sounds simple and is not, because "today" has to
be decided somewhere and every obvious source is wrong in a different way.

Suppliers commonly put the date directly in the filename, and when they do it
is worth far more than the alternatives:

  * a **server mtime** is the time the file was written, which drifts with
    upload duration, retries and the server's own clock
  * a **download time** is about us, not about the data
  * the **file's contents** usually do not say

The filename is the supplier's own statement about which day the data belongs
to, which is exactly the question being asked.

The catch -- and it is the sharpest edge in this module -- is that the date in
the filename is in the SUPPLIER'S calendar, not ours, not the server's, and not
UTC. See :func:`today_in` and :func:`is_from_today`, which take the zone as a
parameter for that reason.

An illustrative listing:

    FULL_FEED_100000_20260901.zip        a full daily snapshot
    FULL_FEED_100000_20260902.zip
    FULL_FEED_100000_20260903.zip

    DELTA_FEED_100000_20260904_80.zip    incremental, sequence 80
    DELTA_FEED_100000_20260904_81.zip
    DELTA_FEED_100000_20260904_82.zip
    ...
    DELTA_FEED_100000_20260904_99.zip
...
_PATTERN = re.compile(
    ...
    [_\-\s]*FEED
    [_\-\s]*
    (?P<account>[A-Za-z0-9]+)? # 100000 -- the vendor's account number for us
    [_\-\s]*
    (?P<date>\d{8})            # 20260904
    (?: [_\-\s]* (?P<seq>\d+) )?   # _80
    (?: \. (?P<ext>[A-Za-z0-9]+) )?  # .zip
...
def parse(filename: str) -> ParsedName:
    ...
    >>> p = parse("FULL_FEED_100000_20260901.zip")
    >>> p.kind.value, p.date_str, p.sequence, p.account
    ('full', '20260901', None, '100000')
    >>> p.recognised
    True

    >>> d = parse("DELTA_FEED_100000_20260904_80.zip")
    >>> d.kind.value, d.date_str, d.sequence
    ('delta', '20260904', 80)

    Case and separators do not matter:

    >>> parse("delta-feed-100000-20260904-7.ZIP").sequence
    7
...
def is_from_today(parsed: ParsedName, timezone_name: str) -> bool:
    ...
    This is the "only today's files" rule. It is an exact
...
def sort_for_processing(names: list[str]) -> list[ParsedName]:
    ...
    ...     "DELTA_FEED_100000_20260904_81.zip",
    ...     "DELTA_FEED_100000_20260904_80.zip",
    ...     "FULL_FEED_100000_20260904.zip",
    ... ])]
    ['FULL_FEED_100000_20260904.zip', 'DELTA_FEED_100000_20260904_80.zip', 'DELTA_FEED_100000_20260904_81.zip']
    """

...
=========================
The shape this parser is built for (a distributor feed is usually some
variation on this):

  * a ZIP archive containing a single text member
  * a full feed running to millions of rows; a delta to tens or hundreds
  * delimited text, frequently pipe-delimited despite being called a CSV::

        barcode|brand|title|price|stock|format
        1115962442528|NORTHWIND|WIDGET STANDARD|12.12|0|STD

  * barcodes arrive with leading zeros stripped (see :mod:`app.core.barcode`)
  * the large majority of rows are ``stock=0`` at any moment, because a
    distributor lists everything it can order, not everything it holds
  * stock values occasionally very large -- a warehouse total rather than
    anything the seller could actually obtain

None of that is assumed rigidly: the delimiter and the column positions are
both settings, because the one thing that is certain about a supplier feed is
that the next supplier's will be different. See :mod:`app.core.settings_store`
and ``docs/ADAPTING.md``.

WHY THIS IS A STREAMING PARSER
==============================
Loading a million rows into a list of dicts costs well over a gigabyte of
memory, and this is expected to run alongside a database on modest hardware. So
nothing here ever holds the whole file: the zip member is read as a stream and
rows are yielded one at a time, so peak memory is a function of the batch size
rather than the feed size. The caller batches them into the database.

A dataframe library would do the same job in comparable time, but a
hand-written stdlib reader is within the same range for flat delimited text and
removes a large dependency from the deployed image. For a single-shape parser
run on a schedule, that trade favours the standard library.
...
EXPECTED_HEADER = ("barcode", "brand", "title", "price", "stock", "format")

#: csv.field_size_limit defaults to 128 KB. Real product titles get long --
#: full descriptions, editions, multi-part names -- though nowhere near that.
#: Raised anyway so that one pathological row cannot abort a million-row
#: import: the cost of the ceiling being generous is nothing, and the cost of
#: it being tight is the whole feed.
...
#:
#: A million-row feed runs to a few hundred megabytes uncompressed. 4 GB is an
#: order of magnitude above that, so it will not trip on genuine growth, while
#: still bounding the work a single archive can ask for.
...
def verify_archive(path: Path) -> tuple[str, int]:
    ...
    A hostile archive is not the primary threat model -- the source is an
    authenticated supplier account. The realistic threat is a mistake: a wrong
    file dropped in the folder, a nested archive, a backup that expands to fill
    the disk and stops the sync. Bounding it costs two comparisons, which is
    cheap enough that the threat model barely matters.
    """
    if not path.exists():
...
def header_matches(observed: str, *, delimiter: str = "|") -> bool:
    ...
    >>> header_matches("barcode|brand|title|price|stock|format")
    True
    >>> header_matches("BARCODE | BRAND | TITLE | PRICE | STOCK | FORMAT")
    True
    >>> header_matches("barcode|brand|title|stock|price|format")   # reordered
    False
...
def iter_rows(
    ...
    *,
    ...
    totals, which avoids a second pass over a million-row file.
    ...
    ("LIFE OF LUCKY CUCUMBER", "GIZMO (LARGE)") and a naive split
    would mangle them.
    """
    ...
    with zipfile.ZipFile(path) as z, z.open(member_name) as raw:
        ...
        for line_no, parts in enumerate(reader, start=2):  # header was line 1
            ...
            yield (
                FeedRow(
                    barcode=bc,
                    brand=_clean(parts[i_artist]) if i_artist is not None else "",
...
#:
#: The rest of the C0 range goes too. They are meaningless inside an brand or
...
def _clean(value: str | None) -> str:
    ...
    column indices they are fed come from the operator-editable ``column_map``
    setting: a mapping that names a column the file does not have would
    otherwise raise mid-parse, hundreds of thousands of rows into a feed -- the
    worst possible moment to discover a configuration error. The annotation
    used to

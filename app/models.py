...
this order:
  ...
  1. ``Setting`` / ``AuditEvent`` / ``User``
     Configuration the operator owns, and the record of who changed what.
...
#: every batch. A busy installation writes millions of rows a year, so the 2.1
#: billion ceiling of a 32-bit key is a real horizon rather than a theoretical
#: one -- and widening a primary key after the fact is an outage.
#:
...
def utcnow() -> datetime:
    ...
    Everything is stored in UTC. "Which day is it" is a *presentation*
    concern, answered with the configured timezone at the edge -- never by
    storing local times, which
...
class MapSource(str, enum.Enum):
    ...
    Anything below MANUAL_OVERRIDE is never pushed. An unverified guess is
    how you update the wrong product -- and because Amazon accepts writes to
    SKUs that do not exist, a wrong guess is not reported as an error.
    """
...
class Setting(Base):
    """
    One operator-editable behaviour knob.
    ...
    operator can change behaviour without a developer.
    ...
    #: Shown next to the field in the dashboard. Written for the operator, not
...
class User(Base):
    ...
    A single shared administrator profile is enough for the common case -- one
    or two people operating one account -- so a fresh install seeds exactly one
    administrator rather than asking for a role design nobody needs yet. The
    ``role``
...
class FeedFile(Base):
    ...
    Illustrative filenames, showing the shape the parser expects:
        FULL_FEED_100000_20260901.zip          (a full daily snapshot; these
                                                run to millions of rows)
        DELTA_FEED_100000_20260904_80.zip      (~4 KB, 105 rows, one every
...
class VendorProduct(Base):
    ...
    A typical feed header, ``barcode|brand|title|price|stock|format``:
      * ``brand`` and ``title`` are descriptive only
      * ``price`` is recorded but NEVER sent to Amazon -- the seller sets their
        own prices with their own margin. See :mod:`app.amazon.guard`.
      * ``format`` is the product type: BULK, STD, CASE, PACK, SINGLE, KIT, SET, ACC ...
    ...
    brand: Mapped[str] = mapped_column(String(400), default="")
    ...
    #: "missing from full feed" setting compares against this, so an operator
    #: can choose "zero it immediately" (1) or "wait for confirmation" (2+).
...
class VendorProductHistory(Base):
    ...
    the raw material for the generated reports.
    """
...
class AmazonListing(Base):
    ...
    A caution about the report this is built from: the columns Amazon returns
    are not guaranteed. A report may contain only
    ``seller-sku, asin1, price, quantity, status`` -- with no product-id and no
    fulfillment-channel column. The code therefore does not assume either is
    present: the barcode is recovered from inside the SKU (see
    :func:`app.core.barcode.split_sku`), and the fulfilment channel is taken
    from the listing itself when it is read back through the Listings API.
    Both are recorded here once known.

    Scope: ``sku_prefix`` is how the system knows which supplier a listing
    belongs to, and therefore whether this feed is entitled to change it. See
    :mod:`app.engine.mapping`.
    """
    ...
    #: equality test rather than a LIKE scan over the whole catalogue.
    ...
    #: only case this system may write to. Amazon also spells this
    #: "Fulfillment by Merchant (Default)" in flat-file exports. Anything
    #: starting with
...
class Run(Base):
    ...
    everything already matched" is information an operator wants on the
    dashboard.
    """
    ...
    #: Full text of the guardrail that stopped the run, in the operator's
    #: language rather than the developer's, e.g. "Would set 2,431 products to
    #: zero; the limit is 500." A halt has to explain itself to the person
...
class ReportFile(Base):
    ...
    Two of these (In Stock and New Products) are useful on their own and

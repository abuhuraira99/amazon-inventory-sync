"""
Assembling the API clients from stored credentials.
...
class ReadinessReport:
    """
    What is configured and what is not, in the operator's language.
...
def vendor_credentials(session: Session) -> VendorCredentials | None:
    ...
    prefers a dashboard entry over any environment value.
    """
    ...
    if mode == "ftp" and not settings.allow_plaintext_ftp:
        ...
        # vendor password on the wire in clear text. Explicit TLS is normally
        # available on the same port, so the downgrade buys nothing.
        log.error(

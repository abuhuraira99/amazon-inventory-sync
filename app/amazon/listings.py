...
==============================================
Every value below was read out of a real Amazon Price & Quantity flat-file
template rather than taken from prose documentation.

That distinction matters more than it looks. The documented attribute names and
the names the API actually accepts are not reliably the same string, and a
wrong one does not raise -- it is accepted and ignored. A downloaded template is
Amazon telling you what it expects, in its own words.

The template's attribute row names::
...
A filled-in row confirms the aliases in practice::

    A7='EXAMPLE-0932826789213' | B7='Fulfillment by Merchant (Default)' | C7='1'

So a quantity patch is::
...
def _quote(sku: str) -> str:
    ...
    URL-safe: ``SUP4-1234567890123-V.G`` has a dot, and there are live SKUs
    with spaces and a leading colon (``": ALT-7263956611093"``) from

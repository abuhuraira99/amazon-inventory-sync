...
They are conservative deliberately. The quota belongs to the seller account,
not to this application, and anything else the seller runs draws on the same
budget -- so consuming all of it would break tools this system knows nothing
about.
...
We run at a fraction of the ceiling (see :data:`RATE_LIMITS`) because the
extra throughput buys nothing here: a large batch goes through the Feeds API in
a single request, and incremental runs are small. Spending the quota would cost
other tools something and gain this one nothing.
...
#:
#: Operators are told to start in practice mode and stay there until they
#: trust the system -- and trust cannot be built in a mode where the comparison
#: never happens. A practice mode that blocks the read it needs is not a
#: cautious practice mode; it is a broken one that reports success.
#:
...
class SpApiClient:
    ...
    def __init__(
        ...
        *,
        ...
        #: When True the HTTP client validates and logs the request but never sends
        ...
        self._client = httpx.Client(
            ...
            headers={"user-agent": "amazon-inventory-sync/1.0 (Language=Python)"},
        )
        ...
        *,
        ...
        if is_write and not allow_write:
            raise SpApiError(
                ...
                "the seller's account must declare that it intends to.",
                operation=operation,
            )
...
def _explain_error(operation: str, response: httpx.Response, errors: list[dict]) -> str:
    ...
    known = {
        "8684": (
            ...
            "update will work until then."
        ),
        "13013": (
            "Amazon says the product is not in its catalogue, so an offer cannot be "
            "attached. This is a new-product problem rather than a stock problem: "
            "the product has to exist in Amazon's catalogue before an offer can "
            "carry a quantity."
        ),
        "8560": (
...
def preflight(client: SpApiClient) -> tuple[bool, str]:
    ...
    halfway through a large batch costs a confusing half-applied state that

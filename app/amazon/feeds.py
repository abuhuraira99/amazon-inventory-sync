...
Two paths, two jobs:
  ...
  * :mod:`app.amazon.listings` patches one SKU per HTTP request. Feedback is
    ...
    frequent incremental runs, which are typically a few hundred products at
    most.

  * This module submits the whole batch as ONE document. Right for the daily
    full-feed reconcile, where the change count can run into the thousands.
    At a polite 2 requests per second, a few thousand per-SKU calls take longer
    than the sync interval itself; as a single feed document it is one
    request.
...
The message shape, derived from a real Amazon flat-file template rather than
from prose documentation (see :mod:`app.amazon.listings` for why that
distinction matters)::
...
=============================
A feed can report DONE while having rejected individual rows. Amazon's own
processing report distinguishes "processed", "successful" and "successful with
other errors" -- and that last category is the trap, because those are real
failures wearing a success label.

Treating DONE as success therefore hides exactly the rows that need attention.
:func:`parse_processing_report` extracts the per-SKU outcome so every rejection
is recorded against its own push item.
"""
...
def build_feed_document(items: list[FeedItem], *, seller_id: str) -> bytes:
    ...
    ...     [FeedItem("EXAMPLE-0007298811035", 7),
    ...      FeedItem("EXAMPLE-4705343071514", 0, lead_time_to_ship_days=5)],
...
def parse_processing_report(text: str, items: list[FeedItem]) -> tuple[dict[str, SkuOutcome], dict]:
    ...
    THIS IS WHY WE DO NOT TRUST "DONE". A feed routinely reports more rows
    processed than succeeded, with the difference filed under "successful with
    other errors" -- real failures wearing a success label. Common codes:
    8684 (SKU linked to more than one GCID), 13013 (product not in the
    catalogue), 8560 (identifier not matched).
    ...
    >>> items = [FeedItem("EXAMPLE-0066561013316", 5), FeedItem("EXAMPLE-0007298811035", 3)]
    >>> report = json.dumps({
    ...
    >>> outcomes["EXAMPLE-0066561013316"].accepted, outcomes["EXAMPLE-0066561013316"].code
    (False, '8684')
    >>> outcomes["EXAMPLE-0007298811035"].accepted
    True

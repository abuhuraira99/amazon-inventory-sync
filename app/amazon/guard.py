"""
The payload guard: the last thing between this system and the seller's listings.

WHY THIS FILE EXISTS
====================
This system updates stock quantity and must never touch prices.

That is not merely a feature it does not offer. Pricing is a domain with its
own logic -- shipping, tax, margin, competitive repricing -- usually owned by a
different system or a person, and an inventory sync that silently rewrites a
price destroys work it knows nothing about and cannot reconstruct.

The risk is structural rather than hypothetical, because Amazon's own
interfaces carry quantity and price side by side. The flat-file template used
for quantity updates has both in adjacent columns::

    A7='EXAMPLE-0932826789213' | C7='1' | G7='15.99'
                                 ^qty        ^price
...
============================
Taken from a real Amazon flat-file template rather than from prose
documentation, because the documented names and the accepted names are not
...
#:
#: Deliberately broad, and the asymmetry is the justification. A false
#: positive costs one confused developer five minutes and a stack trace. A
#: false negative silently overwrites a price somebody calculated deliberately,
#: with no error anywhere, and may not be noticed for weeks.
_FORBIDDEN_SUBSTRINGS: tuple[str, ...] = (
    ...
    "amount",
    "money",
    "tax",                # tax rate / tax code: never ours to set
    "business_price",
    "sale_price",
...
def assert_quantity_only(payload: Any, *, context: str = "") -> None:
    ...
    if offenders:
        ...
        msg = (
            ...
            "stock quantity. Prices are set elsewhere, deliberately, and are "
            "never this system's to write. Nothing has been sent to Amazon. "

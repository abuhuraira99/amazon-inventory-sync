...
def _build_message(note: Notification) -> EmailMessage:
    ...
    msg["From"] = formataddr(("Amazon Inventory Sync", settings.smtp_from))
...
def build_daily_digest(session: Session) -> tuple[str, str]:
    ...
    if not runs:
        return (
            "Amazon Inventory Sync: nothing ran in the last 24 hours",
    ...
    if failed or halted:
        subject = f"Amazon Inventory Sync: {len(failed) + len(halted)} run(s) need attention"
    elif waiting:
        subject = f"Amazon Inventory Sync: {len(waiting)} batch(es) waiting for approval"
    else:
        subject = f"Amazon Inventory Sync: {pushed:,} quantities updated in the last 24 hours"

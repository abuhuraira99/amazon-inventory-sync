...
Amazon *also* refuses — two independent locks on the promise that matters most
about.
...
The LWA credentials screen shows a **rotation deadline** for your Client Secret.
Note the date it gives you and set a reminder a few weeks before it.

This is worth diarising because of how it fails. After the deadline the secret
simply stops working and Amazon returns `invalid_client` — which looks exactly
like a bug in your software, on a system that has been running untouched for
months. Nothing changed on your side, so every obvious place to look is the
wrong one.

When the reminder fires: click **Rotate secret**, then update it in the
dashboard. It is the only hard expiry date in the system.
...
send it.** Guessing at a credential is how accidents happen, and a screenshot takes
five seconds.

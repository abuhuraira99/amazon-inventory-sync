...
# environment variable, so a file that reads only os.environ works there and
# only there. Run the same command directly on a host -- which is what the
# non-container deployment path does -- and DATABASE_URL is simply absent,
# because it lives in .env and nothing has loaded it. Every migration then
# fails with "DATABASE_URL is not set". The
...
def run_migrations_offline() -> None:
    ...
    Useful when a DBA wants to review the statements before they are
